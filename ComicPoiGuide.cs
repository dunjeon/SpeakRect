...
{
    /// <summary>
    /// Green region boxes ± outside fog on tone, plus orange island canvases for Speak.
    /// </summary>
    public static class ComicPoiGuide
    {
        /// <summary>Same text as <see cref="AppSettings.DefaultOcrPrompt"/>.</summary>
        ...
        /// <summary>
        /// Grow <paramref name="hole"/> to min width/height inside the free band
        /// (frame + neighbor gaps), always containing the tight island. Prefer
        /// center; if the bottom is blocked, grow up.
        /// </summary>
            ...
            int neighborGapPx = IslandExpandNeighborGapPx)
        {
            ...
            if (avoidIslands != null && avoidIslands.Count > 0)
            {
                ...
                // Bottom blocked → reclaim height upward (and vice versa).
                expanded = RecoverMinSizeInFreeBand(
                    tight, expanded, needW, needH,
                    limitL, limitT, limitR, limitB, frameW, frameH);
                // Recover stays inside the free band; clamp again in case of diagonals.
                expanded = ClampCropAwayFromNeighbors(
                    tight, expanded, avoidIslands, frameW, frameH, neighborGapPx);
            }
